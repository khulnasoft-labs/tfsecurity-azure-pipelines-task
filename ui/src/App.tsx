...
import {ResultSet} from './tfsecurity'
...
export class App extends React.Component<AppProps, AppState> {
    ...
    async check() {
        ...
        timeline.records.forEach(function (record: TimelineRecord) {
            if (record.type == "Task" && record.task !== null && record.task.name == "tfsecurity") {
                recordId = record.id
        ...
        if (attachments.length === 0) {
            this.setState({error: "No attachments found: cannot load results. Did tfsecurity run properly?"})
            return
        }
